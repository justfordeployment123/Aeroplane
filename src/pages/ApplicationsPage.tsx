...
// Define the type since the array is now generated inside the component
interface Scenario {
    id: string;
    num: string;
    title: string;
...
const ImmersiveScenario = ({ scenario, index }: { scenario: Scenario; index: number }) => {
    ...
    return (
        <motion.div ref={sectionRef} style={{ opacity }} className="relative">
            <div
            ...
            <div className={`relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center ${!isEven ? "direction-rtl" : ""}`}>
                <motion.div className={`relative ${!isEven ? "lg:order-2" : ""}`} style={{ y: imgY }}>
                        ...
                        >
                            ...
                            <div className="absolute top-5 left-5 flex items-center gap-2 z-20">
                                ...
                                <span className="text-[10px] font-mono uppercase tracking-widest" style={{ color: scenario.accent }}>
                                    {t("scenariosData.scenarioText")} {scenario.num}
                                </span>
                            </div>
                        </motion.div>
                    </div>
                ...
                <motion.div className={`flex flex-col gap-8 ${!isEven ? "lg:order-1 lg:text-right lg:items-end" : ""}`} style={{ y: textY }}>
                    <motion.div
                        ...
                        {scenario.stats.map((stat, i) => (
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                    <div className="text-2xl font-black mb-1" style={{ color: scenario.accent }}>
                                        {stat.value}
                                    </div>
                    ...
                    <Link to={`/applications/${scenario.id}`}>
                        ...
                        >
                            ...
                            <ArrowRight
                                size={16}
                                className={`group-hover:translate-x-1 transition-transform ${!isEven ? "rotate-180 group-hover:-translate-x-1" : ""}`}
                            />
                        </motion.button>
                    </Link>
                </motion.div>
...
export const ApplicationsPage = () => {
    ...
    // Combine static config (icons, colors, ids) with dynamic translation text
    const scenarios: Scenario[] = useMemo(() => {
        // Fallbacks for type safety with arrays returned from JSON
        const civilStats = t("scenariosData.items.civil-logistics.stats", { returnObjects: true });
        const emergencyStats = t("scenariosData.items.emergency-rescue.stats", { returnObjects: true });
        const industryStats = t("scenariosData.items.industry-service.stats", { returnObjects: true });
        const fireStats = t("scenariosData.items.forest-fire-prevention.stats", { returnObjects: true });

        return [
            {
                id: "civil-logistics",
                num: "01",
                title: t("scenariosData.items.civil-logistics.title"),
                label: t("scenariosData.items.civil-logistics.label"),
                desc: t("scenariosData.items.civil-logistics.desc"),
                img: "/images/applications/civil-logistics.jpg",
                icon: Package,
                accent: "#00d2ff",
                stats: Array.isArray(civilStats) ? civilStats : [],
            },
            {
                id: "emergency-rescue",
                num: "02",
                title: t("scenariosData.items.emergency-rescue.title"),
                label: t("scenariosData.items.emergency-rescue.label"),
                desc: t("scenariosData.items.emergency-rescue.desc"),
                img: "/images/applications/emergency-rescue.jpg",
                icon: ShieldAlert,
                accent: "#ef4444",
                stats: Array.isArray(emergencyStats) ? emergencyStats : [],
            },
            {
                id: "industry-service",
                num: "03",
                title: t("scenariosData.items.industry-service.title"),
                label: t("scenariosData.items.industry-service.label"),
                desc: t("scenariosData.items.industry-service.desc"),
                img: "/images/applications/industry-service.jpg",
                icon: Factory,
                accent: "#f59e0b",
                stats: Array.isArray(industryStats) ? industryStats : [],
            },
            {
                id: "forest-fire-prevention",
                num: "04",
                title: t("scenariosData.items.forest-fire-prevention.title"),
                label: t("scenariosData.items.forest-fire-prevention.label"),
                desc: t("scenariosData.items.forest-fire-prevention.desc"),
                img: "/images/applications/forest-fire-prevention.png",
                icon: TreePine,
                accent: "#22c55e",
                stats: Array.isArray(fireStats) ? fireStats : [],
            },
        ];
    }, [t]);
    ...
    return (
        <div className="min-h-screen text-white font-sans" style={{ background: "#161622" }}>
            ...
            <section className="relative h-[65vh] min-h-[520px] flex items-center justify-center overflow-hidden">
                ...
                <div
                    className="absolute inset-0"
                    style={{ background: "linear-gradient(to top, #161622 10%, rgba(22,22,34,0.7) 50%, transparent 100%)" }}
                />
                ...
                <div className="relative z-10 text-center px-6">
                    <motion.div initial={{ y: 30 }} animate={{ y: 0 }} transition={{ duration: 0.9 }}>
                        ...
                        <p className="text-gray-300 text-lg max-w-lg mx-auto mt-4">{t("hero.subtitle")}</p>
                    </motion.div>
                </div>
